...
namespace ProjectsManager
{
    public partial class Form1 : Form
    {
        ...
        public void showFinance()
        {
            ...
            DataTable dtIncomesDefinite = mainInterface.Select("SELECT bills.id,bills.payDate,(projects.amount * bills.curencyRate * bills_projects.percent / 100) AS amount,projects.projectName,bills.idBill,bills.bank,bills.curencyRate,bills.increase FROM bills INNER JOIN bills_projects ON bills.id=bills_projects.idBill INNER JOIN projects ON bills_projects.idProject=projects.idProjects WHERE (bills.payDate IS NOT NULL AND bills.payDate>='" + MyUtills.dateToSQL(updatedDate) + "') GROUP BY bills_projects.idBill ORDER BY bills.payDate ASC").Tables[0];
            DataTable dtIncomesForecast = mainInterface.Select("SELECT bills.id,bills.toPayDate,(projects.amount * bills.curencyRate * bills_projects.percent / 100) AS amount,projects.projectName,bills.idBill,bills.bank,bills.curencyRate,bills.increase FROM bills INNER JOIN bills_projects ON bills.id=bills_projects.idBill INNER JOIN projects ON bills_projects.idProject=projects.idProjects WHERE (bills.toPayDate IS NOT NULL AND bills.payDate IS NULL AND bills.toPayDate>='" + MyUtills.dateToSQL(updatedDate) + "') GROUP BY bills_projects.idBill ORDER BY bills.toPayDate ASC").Tables[0];
